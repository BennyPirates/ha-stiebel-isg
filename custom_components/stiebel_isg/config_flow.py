...
from .const import CONF_OFFSET, CONF_UNIT_ID, DEFAULT_PORT, DEFAULT_TIMEOUT, DEFAULT_UNIT_ID, DOMAIN
...
class StiebelIsgConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict | None = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            client = StiebelIsgClient(user_input[CONF_HOST], user_input[CONF_PORT], user_input[CONF_UNIT_ID], DEFAULT_TIMEOUT)
            try:
                offset = await client.detect_offset()
            except StiebelIsgError:
                errors["base"] = "cannot_connect"
            except Exception:  # Home Assistant config flows must not expose library errors.
                errors["base"] = "unknown"
            else:
        ...
        schema = vol.Schema({
            ...
            vol.Required(CONF_UNIT_ID, default=DEFAULT_UNIT_ID): vol.All(vol.Coerce(int), vol.Range(min=0, max=247)),
        })
