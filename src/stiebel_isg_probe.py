...
UNAVAILABLE = 0x8000
DOC_URL = "https://www.stiebel-eltron.de/static/ste/docportal/manual/DM0000040850-hqy.pdf"
...
for address in range(501, 609):
    if ("input", address) not in _existing:
        data_type = 7 if address in {538, 540, 541, 545, 546, 547, 552, 553, 554,
                                     559, 560, 561, 566, 567, 568, 573, 574, 575,
                                     580, 581, 582} else (6 if address in {530, 535} else 2)
        REGISTERS.append(Register(address, "input", "Documented WPM system value",
                                  data_type, meaning="documented_unlabelled"))
for start, end in ((2501, 2547), (3501, 3655)):
    for address in range(start, end + 1):
        if ("input", address) not in _existing:
            REGISTERS.append(Register(address, "input", "Documented WPM status/energy value",
                                      6, meaning="documented_unlabelled"))
...
STATUS_BITS = {
    0: "HC1 pump", 1: "HC2 pump", 2: "heat-up program", 3: "backup heater",
    4: "heating", 5: "DHW", 6: "compressor", 7: "summer mode",
    8: "cooling", 9: "defrost", 10: "silent 1", 11: "silent 2",
}
...
class ReadOnlyProbe:
    ...
    def read_one(self, kind: str, pdu_address: int) -> int:
        ...
        method = (self.client.read_input_registers if kind == "input"
...
def score_offset(probe: ReadOnlyProbe, offset: int) -> tuple[int, list[str]]:
    ...
    for address in OFFSET_PROBES:
        ...
        try:
            ...
            notes.append(f"{address}: {value} {reg.unit} ({'ok' if ok else 'implausible'})")
...
def make_result(reg: Register, offset: int, raw: int | None, error: str | None = None) -> Result:
    if error:
        return Result(reg.address, reg.address + offset, reg.kind, 4 if reg.kind == "input" else 3,
                      reg.name, None, None, reg.data_type, TYPE_SCALE.get(reg.data_type), None,
                      reg.unit, "error", "unknown", reg.meaning, error)
    assert raw is not None
    if raw == UNAVAILABLE:
        return Result(reg.address, reg.address + offset, reg.kind, 4 if reg.kind == "input" else 3,
                      reg.name, raw, f"0x{raw:04X}", reg.data_type, TYPE_SCALE.get(reg.data_type),
                      None, reg.unit, "unavailable", "not_applicable", reg.meaning)
    if reg.address in {517, 1508} and raw == 0x9000:
        return Result(reg.address, reg.address + offset, reg.kind, 4 if reg.kind == "input" else 3,
                      reg.name, raw, f"0x{raw:04X}", reg.data_type, TYPE_SCALE.get(reg.data_type),
                      "off", reg.unit, "available", "plausible", reg.meaning)
    value = decode(raw, reg.data_type)
    return Result(reg.address, reg.address + offset, reg.kind, 4 if reg.kind == "input" else 3,
                  reg.name, raw, f"0x{raw:04X}", reg.data_type, TYPE_SCALE[reg.data_type],
                  value, reg.unit, "available", "plausible" if plausible(reg, value) else "implausible",
                  reg.meaning)


def candidate_registers() -> Iterable[Register]:
    # Narrow gaps/edges next to official WPM blocks; semantics deliberately unknown.
    for start, end, kind in [(609, 620, "input"), (1522, 1530, "holding"),
                             (2548, 2560, "input"), (3656, 3670, "input")]:
        for address in range(start, end + 1):
            yield Register(address, kind, "Unknown responding register candidate", 6,
                           meaning="candidate")


def write_outputs(results: list[Result], metadata: dict, output_dir: Path) -> tuple[Path, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().astimezone().strftime("%Y%m%d_%H%M%S")
    json_path = output_dir / f"stiebel_probe_{stamp}.json"
    csv_path = output_dir / f"stiebel_probe_{stamp}.csv"
    json_path.write_text(json.dumps({"metadata": metadata, "registers": [asdict(r) for r in results]},
...
def print_results(results: list[Result]) -> None:
    print("\nAddr  Art      Rohwert  Wert          Einheit  Verfügbarkeit  Plausibilität  Bezeichnung")
    print("-" * 112)
    for r in results:
        raw = "-" if r.raw_value is None else str(r.raw_value)
        value = "-" if r.decoded_value is None else str(r.decoded_value)
        print(f"{r.address:5}  {r.register_type:7}  {raw:7}  {value:12}  {r.unit:7}  "
              f"{r.availability:14} {r.plausibility:15} {r.name}")
        if r.address == 2501 and isinstance(r.decoded_value, int):
            active = [name for bit, name in STATUS_BITS.items() if r.decoded_value & (1 << bit)]
            print("       Aktive Statusbits: " + (", ".join(active) or "keine"))
    counts = {state: sum(r.availability == state for r in results)
              for state in ("available", "unavailable", "error")}
    implausible = sum(r.plausibility == "implausible" for r in results)
    print(f"\nZusammenfassung: {counts['available']} verfügbar, {counts['unavailable']} nicht verfügbar, "
          f"{counts['error']} Fehler, {implausible} unplausibel")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read-only STIEBEL ISG Modbus diagnostic probe")
    ...
    parser.add_argument("--offset", type=int, choices=(-1, 0), help="override automatic offset detection")
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"Invalid/unresolvable host {args.host!r}: {exc}")
    ...
    try:
        ...
        if args.offset is None:
            ...
            for candidate in (-1, 0):
                print(f"  Offset {candidate:+}: score {scores[candidate]} — " + "; ".join(notes[candidate]))
            if scores[-1] == scores[0] or max(scores.values()) <= 0:
                raise SystemExit("Offset cannot be determined safely; use --offset only after reviewing readings")
    ...
    metadata = {
        "timestamp_utc": datetime.now(timezone.utc).isoformat(), "host": args.host,
        "port": args.port, "unit_id": args.unit, "offset": offset,
        "offset_scores": scores, "read_only": True, "function_codes": [3, 4],
        "candidate_scan": args.scan_candidates, "documentation": DOC_URL,
    }
